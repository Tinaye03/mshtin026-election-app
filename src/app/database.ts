...
class Database {
    ...
    async addVoter(voter: Voter): Promise<void> {
        ...
        const voterRef = doc(collection(firestore, 'voters'), voter.idNumber);
    ...
    async updateVoterStatus(voter: Voter): Promise<void> {
        // Create a reference to a new document with the specified ID
        const voterRef = doc(collection(firestore, 'voters'), voter.idNumber);
        console.log('voterRef', voterRef);
        ...
        try {
            ...
            console.log('Voter status updated for user ${userId}');
        } catch (error) {
